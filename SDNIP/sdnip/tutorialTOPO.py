#!/usr/bin/python
#sudo mn --custom ~/tutorial.py --topo sdnip --controller remote --nolistenport

from mininet.topo import Topo
from mininet.net import Mininet
from mininet.cli import CLI
from mininet.log import setLogLevel, info, debug
from mininet.node import Host, RemoteController, OVSBridge, NAT
from ipaddress import ip_network


QUAGGA_DIR = '/quagga/quagga-0.99.23/'
...
class Router(Host):
    ...
    def config(self, **kwargs):
        ...
        self.cmd('/quagga/quagga-0.99.23/zebra/zebra -d -f %s -z %s/zebra%s.api -i %s/zebra%s.pid' % (self.zebraConfFile, QUAGGA_RUN_DIR, self.name, QUAGGA_RUN_DIR, self.name))
        print('\n%s /quagga/quagga-0.99.23/zebra/zebra -d -f %s -z %s/zebra%s.api -i %s/zebra%s.pid' % (self.name,self.zebraConfFile, QUAGGA_RUN_DIR, self.name, QUAGGA_RUN_DIR, self.name))
        
        self.cmd('/quagga/quagga-0.99.23/bgpd/bgpd -d -f %s -z %s/zebra%s.api -i %s/bgpd%s.pid' % (self.quaggaConfFile, QUAGGA_RUN_DIR, self.name, QUAGGA_RUN_DIR, self.name))
        print('%s /quagga/quagga-0.99.23/ospfd/bgpd -d -f %s -z %s/zebra%s.api -i %s/bgpd%s.pid' % (self.name,self.quaggaConfFile, QUAGGA_RUN_DIR, self.name, QUAGGA_RUN_DIR, self.name))
